...
BINARY="https://microtick.com/releases/mainnet/microtick-v2.0.2-linux-x86_64.tar.gz"
GENESIS="https://raw.githubusercontent.com/microtick/stargate-genesis/master/genesis.json"
APP="Microtick: ~/.microtick"

read -p "$APP folder, your keys and config will be erased, proced (y/n)? " -n 1 -r
if [[ $REPLY =~ ^[Yy]$ ]]
then
  # Microtick State Sync client config.
  rm -f microtick-v2.0.2-linux-x86_64.tar.gz #deletes a previous downloaded tar
  rm -f mtm #deletes a previous downloaded binary
  rm -rf $HOME/.microtick/ #deletes previous installation   
  wget -nc $BINARY
  tar -xzvf microtick-v2.0.2-linux-x86_64.tar.gz 
  chmod +x mtm
  ./mtm init New_peer --chain-id microtick-1
  ...
  #DOMAIN_3=microtick.zone     # If you want to use domain names 
